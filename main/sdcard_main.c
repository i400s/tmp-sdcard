...
void app_main(void)
{
    ...
    typedef union {
        ...
        struct {
            ...
            uint16_t temp_heat;
            uint16_t temp_cool;
            uint8_t temp_heating_on : 1;
            uint8_t temp_cooling_on : 1;
        } log_data;
    } log_record_t;
    static_assert(sizeof(log_record_t) == 32, "log_record_t size must be 32 bytes");

    static int buff_sz = 16 * sizeof(log_record_t);
    ...
    const char *file_log = SDCARD_MOUNT_POINT"/logfile.txt";

    ESP_LOGI(TAG, "Checking existence of file %s", file_log);
    ...
    struct stat stats;
    stat(file_log, &stats);
    float st_size = (float)stats.st_size / 1073741824;
    ESP_LOGI(TAG, "File size is %5.10f TiB. ", st_size);
    if (st_size > 1) {
        ESP_LOGE(TAG, "File is bigger than 1 GiB");
        return;
    }
    ...
    for (int i = 0; i < 100; i++) {
        ...
        for (int j = 0; j < 16; j++) {
            ...
            log_record.log_data.temp_ambient = j + 1;
            log_record.log_data.temp_heat = i + 1;
            log_record.log_data.temp_cool = 0xFFFF;
            log_record.log_data.temp_cooling_on = true;
            log_record.log_data.temp_heating_on = true;
            gettimeofday(&log_record.log_data.time_now, NULL);

            // place X elements in to buffer

            printf("i:%03d j:%02d timeofday:%jd:%li\n",
                i,
                j,
                (intmax_t)log_record.log_data.time_now.tv_sec,
                log_record.log_data.time_now.tv_usec);
            memcpy(buff + (sizeof(log_record) * j), &log_record, sizeof(log_record));
        ...
        if (!once) {
            ...
            ret = fseek(f, -buff_sz, SEEK_END);
        }
        if (ret) {
            ESP_LOGE(TAG, "Error seeking end of file");
            return;
        };

        if (fwrite(buff, 1, buff_sz, f) != buff_sz) {
            ESP_LOGE(TAG, "Error writing to file");
            return;
        };
        memset(buff, 0xFF, buff_sz);
        if (fwrite(buff, 1, buff_sz, f) != buff_sz) {
            ESP_LOGE(TAG, "Error writing to file");
            return;
        }
    ...
    ESP_LOGI(TAG, "Opening for audit");

    FILE *fp;
    int c;
    fp = fopen(file_log,"rb");
    bool abort = false;
    while(!abort) {
        for (int k = 0; k < 512; k++) {
            c = fgetc(fp);
    ...
    s_sdcard_example_esp_vfs_fat_sdspi_unmount(mount_point, card);
    ESP_LOGI(TAG, "Card unmounted");

    spi_bus_remove_device(touch_handle);
    touch_handle = NULL;
    spi_bus_remove_device(lcd_handle);
    lcd_handle =NULL;
